...
import { supabase, isSupabaseConfigured, type AnotacaoReuniao } from "@/lib/supabase"
import { useToast } from "@/hooks/use-toast"

interface AnotacoesReuniaoProps {
  reuniaoId: number
}

export function AnotacoesReuniao({ reuniaoId }: AnotacoesReuniaoProps) {
  const [anotacoes, setAnotacoes] = useState<AnotacaoReuniao[]>([])
  const [isDialogOpen, setIsDialogOpen] = useState(false)
  const [editingAnotacao, setEditingAnotacao] = useState<AnotacaoReuniao | null>(null)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!isSupabaseConfigured) {
        ...
        localStorage.setItem(`anotacoes_reuniao_${reuniaoId}`, JSON.stringify(anotacoes))
  ...
  const handleEdit = (anotacao: AnotacaoReuniao) => {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      const { error } = await supabase.from("anotacoes_reunioes").delete().eq("id", id)
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>Adicione observações importantes sobre a reunião</DialogDescription>
            </DialogHeader>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <Textarea
                  ...
                  placeholder="Descreva os pontos importantes, decisões tomadas, próximos passos..."
                  rows={6}
                  required
                />
              </div>
